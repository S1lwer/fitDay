    ...
    const Form = () => {
     ...
     {modal? <Modal fn={setModal} />:''}  
                ...
                <div className='form-user-id'>
                    ...
                    <button onClick={(e)=>{e.preventDefault();    setModal(!modal); setIsReady(true)}} id='tme_connect'><Workflow/>Connect</button>
                </div>
                
                <button
                disabled={!isReady}
                onClick={fetchInfo} className={`form-generate-btn ${isReady?'':'disabled'}`}>
